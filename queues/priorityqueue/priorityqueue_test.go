...
func TestBinaryQueueEnqueueBulk(t *testing.T) {
	queue := NewWith(utils.NumberComparator[int])
...
func TestBinaryQueueIteratorLast(t *testing.T) {
	tree := NewWith(utils.NumberComparator[int])
	it := tree.Iterator()
...
func TestBinaryQueueSerialization(t *testing.T) {
	...
	_, err = json.Marshal([]interface{}{"a", "b", "c", queue})
	if err != nil {
		t.Errorf("Got error %v", err)
	}
...
func TestBTreeString(t *testing.T) {
	c := NewWith(utils.NumberComparator[int])
	c.Enqueue(1)
